...
namespace MiBandApp.Services
{
  public class EmailComposer
  {
    ...
    public async void ComposeEmail(IStorageFile file = null, string theme = null)
    {
      ...
      stringBuilder.Append(string.Format(" (User {0} Version {1}", (object) this._settings.GetUserId(), (object) this._settings.AppVersion));
      if (this._bandController.DeviceInfo.Value != null)
        stringBuilder.Append(string.Format(" {0} {1}", (object) this._bandController.DeviceInfo.Value.Model, (object) this._bandController.DeviceInfo.Value.FirmwareVersion));
      if (SystemInfo.SystemVersion != null)
        stringBuilder.Append(string.Format(" {0}", (object) SystemInfo.SystemVersion));
      stringBuilder.Append(")");
      EmailMessage emailMessage = new EmailMessage();
      emailMessage.put_Subject(stringBuilder.ToString());
      emailMessage.To.Add(new EmailRecipient("support@bindmiband.com", "Bind Mi Band Support"));
      if (file != null)
      {
        emailMessage.Attachments.Add(new EmailAttachment(((IStorageItem) file).Name, (IRandomAccessStreamReference) file));
        emailMessage.put_Body(string.Format("<{0}>\n\n", (object) this._resourceLoader.GetString("AboutPage_DiagnosticPackageMessage_EmailHint")));
      }
